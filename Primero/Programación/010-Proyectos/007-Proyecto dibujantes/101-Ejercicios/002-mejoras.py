...
CHANNEL_VIDEOS_URL = CHANNEL_URL.rstrip("/") + "/videos"   # <- clave para listar correctamente
...
# Si True: re-procesa aunque exista en DB.
# Si False: incremental (no reprocesa).
FORCE_REFRESH = False
# =========================
...
CREATE TABLE IF NOT EXISTS videos (
  ...
  thumbnail_url       TEXT,   -- URL original (YouTube/CDN)
  thumbnail_path      TEXT,   -- ruta local
  thumbnail_local_url TEXT,   -- file:///... (comodidad)
...
def to_file_url(abs_path: str) -> str:
    # file:// URL simple (sin depender de pathlib)
    # en linux: file:///ruta/absoluta
...
def upsert_video(
    ...
    thumbnail_local_url: Optional[str],
    ...
    conn.execute(
        ...
        ON CONFLICT(video_id) DO UPDATE SET
          ...
          thumbnail_local_url=COALESCE(excluded.thumbnail_local_url, videos.thumbnail_local_url),
...
def extract_video_id_from_entry(entry: Dict[str, Any]) -> Optional[str]:
    """
    En listados 'flat' de yt-dlp, a veces entry['id'] es "UC...." (canal) o cosas raras.
    Preferimos extraer de la URL watch?v=.
    """
    ...
    if isinstance(vid, str) and vid.strip():
        vid = vid.strip()
        # Si empieza por UC, es canal, no vídeo
...
def collect_video_ids_from_channel() -> List[str]:
    """
    Lista vídeos del canal usando /videos y --flat-playlist,
    filtrando cualquier entrada que no sea vídeo.
    """
    data = run_yt_dlp_dump_json(CHANNEL_VIDEOS_URL, flat=True)
    ...
    for e in entries:
        if not isinstance(e, dict):
            continue
        vid = extract_video_id_from_entry(e)
        if not vid:
            continue
        if vid in seen:
            continue
...
def main() -> int:
    ...
    video_ids = collect_video_ids_from_channel()
    if not video_ids:
        print("No se encontraron vídeos (revisa canal o yt-dlp).")
        return 2
    ...
    print(f"Listado: {CHANNEL_VIDEOS_URL}")
    ...
    for idx, video_id in enumerate(video_ids[:limit], start=1):
        ...
        if DOWNLOAD_THUMBNAILS and thumb_url:
            ...
            if thumb_path and os.path.exists(thumb_path):
                thumb_local_url = to_file_url(thumb_path)
        ...
        upsert_video(
            ...
            thumbnail_local_url=thumb_local_url,
