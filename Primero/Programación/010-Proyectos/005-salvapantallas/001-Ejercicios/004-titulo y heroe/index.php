...
/*
  index.php
  - Reads JSON generated by your yt-dlp script
  - HERO slider: playlists (title + description + background thumbnail)
  - Renders each playlist as a <section>
  - Renders each video as an <article> with thumbnail + title
  - On click: opens a floating modal card
*/
...
<html lang="es">
  ...
  <body>
    <nav>
      <div style="font-weight:700;margin-bottom:10px;">Menú</div>
    ...
    <header>
      <h1 style="font-size:20px;">ScreenSaver.es</h1>
      <h2>Save your screen from boredom</h2>
      <div id="hamburguesa" style="font-size:22px;cursor:pointer;">☰</div>
    </header>
    ...
    <main>
      <?php if (!$playlists): ?>
        <div class="empty">
          ...
          Asegúrate de que el JSON esté en: <code><?= h($jsonFile) ?></code>
        </div>
      <?php else: ?>
        ...
        <div class="hero" id="hero">
          <div class="heroSlides" id="heroSlides">
            <?php foreach ($playlists as $idx => $pl): ?>
              <?php
                $plTitle = (string)($pl['title'] ?? 'Sin título');
              ...
              <div class="heroSlide <?= $activeClass ?>"
                ...
                <div class="heroInner">
                  <div class="heroText">
                    <div class="heroKicker">Playlist</div>
                    <div class="heroTitle"><?= h($plTitle) ?></div>
                    <div class="heroDesc"><?= h(trim($plDesc) ?: "Sin descripción.") ?></div>
                    <div class="heroActions">
                      <a class="btn btnPrimary" href="<?= h($plUrl) ?>" target="_blank" rel="noopener">Ver playlist en YouTube</a>
                      <div class="btn" data-scroll-to="<?= h($plTitle) ?>">Ver vídeos abajo</div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <?php foreach ($playlists as $pl): ?>
        ...
        ?>
        <section data-pl-title="<?= h($plTitle) ?>">
          <h3><?= h($plTitle) ?></h3>
          ...
          <div class="contenedor" style="left:0px;">
            <?php if (!$videos): ?>
              <article style="width:300px;height:120px;align-items:center;">
                <div class="meta">
                  <div class="vtitle">Sin vídeos</div>
                  <div class="vlink">—</div>
                </div>
              </article>
            <?php endif; ?>
            ...
            <?php foreach ($videos as $v): ?>
              <?php
                $vttl  = (string)($v['title'] ?? 'Sin título');
                $vurl  = (string)($v['url'] ?? '#');
    ...
    <div class="modalOverlay" id="modalOverlay" aria-hidden="true">
      <div class="modalCard" role="dialog" aria-modal="true" aria-label="Detalles del vídeo" id="modalCard">
        <div class="modalMedia">
          <img id="modalThumb" alt="Miniatura del vídeo" src="">
        </div>
        <div class="modalBody">
          <div class="modalTop">
            ...
            <div class="modalClose" id="modalClose" title="Cerrar">✕</div>
          </div>
    ...
    <script>
      ...
      function openModal({title, desc, url, thumb}){
        mTitle.textContent = title || "Sin título";
        mUrl.textContent   = url || "";
        mGo.href           = url || "#";

        const d = (desc || "").trim();
        mDesc.textContent = d ? d : "Sin descripción.";
      ...
      // "Ver vídeos abajo" button: scroll to matching section title
      document.querySelectorAll(".heroActions [data-scroll-to]").forEach(function(b){
        b.addEventListener("click", function(){
          const title = this.getAttribute("data-scroll-to") || "";
          const sec = document.querySelector('section[data-pl-title="'+CSS.escape(title)+'"]');
