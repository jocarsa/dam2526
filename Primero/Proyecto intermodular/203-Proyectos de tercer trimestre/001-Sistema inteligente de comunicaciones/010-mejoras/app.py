...
""".strip()
    ...
    try:
        ...
        pr = 5
    pr = max(0, min(10, pr))
    reason = " ".join(str(obj.get("reason", "")).split()).strip() or "Sin motivo proporcionado."
    return {"priority": pr, "reason": reason}

# ==========================================================
# RESPUESTA AUTOMÁTICA (SOLO TEXTO)
# ==========================================================
def llm_draft_reply(sender: str, subject: str, date: str, summary: str, body: str) -> str:
    body = (body or "").strip()
    if len(body) > 3000:
        body = body[:3000] + "…"

    prompt = f"""
Eres un asistente que redacta borradores de respuesta a correos.
Devuelve SOLO el texto del correo (sin asunto, sin "De:", sin encabezados técnicos).
Tono: profesional, claro, breve y útil. Español.
...
Reglas:
- Si faltan datos, pide la información mínima necesaria con 1-2 preguntas.
- Si hay una acción solicitada, confirma y propone siguiente paso.
- No inventes hechos.
""".strip()
...
def fetch_and_classify(n: int) -> dict:
    ...
    for msg_id in reversed(ultimos_ids):  # más reciente primero
    ...
    return {"A": categoria_A, "B": categoria_B, "by_id": by_id}

def ensure_cache(n: int):
    if CACHE["items"] is None or CACHE["last_n"] != n:
...
def index():
    ...
    if tab not in ("A", "B"):
        tab = "B"
    # (1) devolvemos página con loader, el contenido real llega por fetch
    return render_template("loading_index.html", tab=tab, n=n)
...
def detail(email_id: str):
    n = int(request.args.get("n", str(N_ULTIMOS)))
    n = max(1, min(200, n))
    return render_template("loading_detail.html", email_id=email_id, n=n)

@app.route("/email_data/<email_id>")
def email_data(email_id: str):
    n = int(request.args.get("n", str(N_ULTIMOS)))
    n = max(1, min(200, n))
...
def draft_reply():
    """
    (2) Genera borrador con Ollama, NO envía.
    Body JSON: {"id":"...","user_text":"..."}
    """
    payload = request.get_json(force=True, silent=True) or {}
    email_id = (payload.get("id") or "").strip()
    user_text = (payload.get("user_text") or "").strip()
    n = int(payload.get("n") or N_ULTIMOS)
    n = max(1, min(200, n))
    ...
    # Prompt adicional: lo que el usuario quiere responder (si ha escrito algo)
    extra = ""
    if user_text:
        extra = f"\n\nPreferencias del usuario para la respuesta:\n\"\"\"{user_text}\"\"\"\n"
