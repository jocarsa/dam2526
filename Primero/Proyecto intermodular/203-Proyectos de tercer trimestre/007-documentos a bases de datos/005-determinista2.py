...
def join_wrapped_field(lines, start_idx, first_line_rest):
    ...
    while i < len(lines):
        ...
        # Si el texto ya termina en punto, suele cerrar el título del módulo
...
def parse_modules(text: str):
    ...
    def flush_current():
        nonlocal current
        if current:
            # Si hay un RA abierto, ciérralo
            if current_ra is not None:
                current["learning_outcomes"].append(current_ra)
            # Derivar unidades/subunidades desde contenidos (determinístico)
    ...
    while i < len(lines):
        ...
        if m:
            ...
            current = {
                ...
                "duration": None,
                "learning_outcomes": [],
                "contenidos_minimos_raw": [],   # líneas crudas desde Contenidos básicos (útil para auditoría)
                "contenidos_minimos": [],       # lista plana de bullets
                "unidades_didacticas": []        # derivado determinísticamente
            }
        ...
        if RE_RA_HEADER.match(s):
            in_ra = True
            in_criterios = False
            # si por algún motivo había RA abierto, ciérralo
            if current_ra is not None:
                current["learning_outcomes"].append(current_ra)
                current_ra = None
            i += 1
        ...
        # Entrar en Contenidos
        if RE_CONTENIDOS.match(s):
            in_contenidos = True
            in_ra = False
            in_criterios = False
            # cerrar RA si estaba abierto
            if current_ra is not None:
                current["learning_outcomes"].append(current_ra)
                current_ra = None
            i += 1
        ...
        if in_ra and not in_contenidos:
            ...
            if m_ra:
                # al iniciar un nuevo RA, guarda el anterior
                if current_ra is not None:
                    current["learning_outcomes"].append(current_ra)
        ...
        # Parse Contenidos básicos (crudo + bullets)
        if in_contenidos:
            # guarda crudo (para auditoría)
...
def derive_units_from_contenidos(raw_lines):
    ...
    for s in raw_lines:
        ...
        if mh:
            ...
            # evita títulos demasiado genéricos o vacíos
