...
# Display only (kept for UI pills)
AI_BACKEND_LABEL = os.environ.get("AI_BACKEND_LABEL", "remote-api")
AI_MODEL_LABEL = os.environ.get("AI_MODEL_LABEL", "ollama-via-php")
...
def call_remote_ai(prompt: str) -> str:
    """
    Calls the remote PHP API (same pattern as generar_explicaciones.py):
    - POST to AI_API_URL
    - header X-API-Key
    - form field 'question'
    - expects JSON with key: 'answer'
    """
    try:
        resp = requests.post(
            AI_API_URL,
...
<div class="wrap">
  <header>
    ...
    <div>
      ...
      <span class="pill">IA: {{backend}}</span>
...
def chat():
    ...
    # 1) Generar SQL (único bloque) con prompt estricto (REMOTE API)
    try:
        llm_text = call_remote_ai(build_prompt(schema_text, q, catalog))
    except Exception as e:
...
def health():
    info = {
        ...
        "ai_backend": AI_BACKEND_LABEL,
        "ai_model_label": AI_MODEL_LABEL,
        "ai_api_url": AI_API_URL,
        "ai_verify_ssl": AI_API_VERIFY_SSL,
        "schema_path": SCHEMA_PATH,
        "allow_dml": ALLOW_DML,
        "strategy": SCHEMA_STRATEGY,
    }

    # Lightweight remote check (may still run the model; keep short).
    try:
        resp = requests.post(
            AI_API_URL,
            headers={"X-API-Key": AI_API_KEY},
            data={"question": "Responde solo con OK."},
            timeout=5,
            verify=AI_API_VERIFY_SSL,
        )
        ok = (resp.status_code == 200)
        info["ai_api_reachable"] = ok
        if ok:
            try:
                data = resp.json()
                info["ai_api_sample"] = (data.get("answer") or "")[:120]
            except Exception:
                info["ai_api_sample_error"] = "Respuesta no-JSON"
        else:
            info["ai_api_error"] = resp.text[:200]
    except Exception as e:
        info["ai_api_reachable"] = False
        info["ai_api_exception"] = str(e)
