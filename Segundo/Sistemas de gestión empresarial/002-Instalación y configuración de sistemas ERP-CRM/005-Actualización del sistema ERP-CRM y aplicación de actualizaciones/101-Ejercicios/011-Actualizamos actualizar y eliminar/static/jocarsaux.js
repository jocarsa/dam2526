...
`;
    ...
    tableRenderer({ target, data, flattenObjects=true, title='jocarsa|ux', subtitle='Table renderer (search + sort + CRUD)', rowsPerPage=10 }={}){
        ...
        inlineForm.style.borderTop = '1px solid var(--jux-line)';
        inlineForm.style.padding = '12px';
        card.append(pad, inlineForm);
        ...
        const paginationDiv = el('div', NS('pagination'));
        card.append(scroll, paginationDiv);
        wrap.append(h1, p, card);
        host.append(wrap);

        this._els = { host, table, thead, tbody, search: input, count, pagination: paginationDiv, inlineForm };
        ...
        // --- helpers for CRUD
        const guessPk = () => {
          if (this.state.columns.includes('id')) return 'id';
          return this.state.columns[0]; // fallback
        };
        const currentTableName = title.split(' - ')[0]; // we pass "<tabla> - Table View" from index.html
        ...
        // --- render Add inline form using same "smart" field analysis
        const renderInlineInsert = () => {
          inlineForm.innerHTML = '';
          const cols = this.state.columns.slice(); // all columns visible; typically include 'id' which may be auto-inc
          const flatRows = this.state.rows;
          const fieldTypes = {};
          cols.forEach(col => {
            const values = flatRows.map(row => row[col]);
            fieldTypes[col] = analyzeField(values);
          });

          const form = document.createElement('form'); form.className = NS('form');
          const grid = document.createElement('div'); grid.className = NS('form-grid'); grid.style.gridTemplateColumns='repeat(2,1fr)';

          cols.forEach(col => {
            // Skip auto PK on insert if it looks like 'id'
            if (col.toLowerCase() === 'id') return;
            const fieldDiv = el('div', NS('field'));
            const label = el('label', NS('label'), col);
            label.htmlFor = `${NAMESPACE}-add-${col}`;
            const fieldInfo = fieldTypes[col];
            let input;
            if (fieldInfo.type === 'select') {
              input = document.createElement('select'); input.className = NS('select');
              const emptyOption = document.createElement('option'); emptyOption.value=''; emptyOption.textContent='-- Select --';
              input.appendChild(emptyOption);
              fieldInfo.options.forEach(opt => { const o=document.createElement('option'); o.value=opt; o.textContent=opt; input.appendChild(o); });
              fieldDiv.append(label, input, el('span', NS('hint'), `${fieldInfo.options.length} options available`));
            } else if (fieldInfo.type === 'textarea') {
              input = document.createElement('textarea'); input.className = NS('textarea');
              fieldDiv.append(label, input, el('span', NS('hint'), 'Long text field'));
            } else if (fieldInfo.type === 'number') {
              input = document.createElement('input'); input.type='number'; input.step='any'; input.className = NS('input');
              fieldDiv.append(label, input, el('span', NS('hint'), 'Numeric field'));
            } else {
              input = document.createElement('input'); input.type='text'; input.className = NS('input');
              fieldDiv.append(label, input, el('span', NS('hint'), 'Text field'));
            }
            input.id = `${NAMESPACE}-add-${col}`;
            input.name = col;
          ...
          cancelBtn.onclick = () => { inlineForm.style.display = 'none'; inlineForm.innerHTML=''; };
          ...
          form.onsubmit = (e) => {
            ...
            postJSON(`/insertar/${encodeURIComponent(currentTableName)}`, 'POST', payload)
              .then(({ok,status,json})=>{
                if (ok && !json.error){
                  alert(`Insertados: ${json.insertados ?? 0}`);
                  // refresh
                  ...
                  alert(`Error (${status}): ${json?.mensaje || 'Error al insertar'}`);
                }
              })
              .catch(err=>{ console.error(err); alert('Fallo de red al insertar'); });
          };
        };

        addBtn.onclick = renderInlineInsert;
        ...
        this._renderBody = () => {
          ...
          pageData.forEach((r, idx)=>{
            ...
            editBtn.type='button';
            editBtn.onclick = () => {
              // simple inline editor via prompts (fast path); could be replaced by modal form if you prefer
              const cambios = {};
              columns.forEach(c=>{
                if (c === pkCol) return;
                const current = r[c] ?? '';
                const v = window.prompt(`Edit ${c}`, String(current));
                if (v !== null) cambios[c] = (v.trim() === '' ? null : v);
              });
              ...
              postJSON(`/actualizar/${encodeURIComponent(currentTableName)}/${encodeURIComponent(pkValue)}`, 'PUT', cambios)
                .then(({ok,status,json})=>{
                  if (ok && !json.error){
                    alert(`Actualizados: ${json.actualizados ?? 0}`);
                    // refresh
                    fetch(`/seleccionar/${encodeURIComponent(currentTableName)}`, { headers: { 'Accept':'application/json' }})
                      .then(r=>r.json())
                      .then(datos=>{
                    ...
                    alert(`Error (${status}): ${json?.mensaje || 'Error al actualizar'}`);
                  }
                })
                .catch(err=>{ console.error(err); alert('Fallo de red al actualizar'); });
            };
            ...
            delBtn.onclick = () => {
              ...
              .then(({ok,status,json})=>{
                if (ok && !json.error){
                  alert(`Eliminados: ${json.eliminados ?? 0}`);
                  // refresh local (remove row) to avoid full reload
                  this.state.rows = this.state.rows.filter(x => String(x[pkCol]) !== String(pkValue));
                  this.setFilter(this.state.query || '');
                } else {
                  alert(`Error (${status}): ${json?.mensaje || 'Error al eliminar'}`);
                }
              })
              .catch(err=>{ console.error(err); alert('Fallo de red al eliminar'); });
            };
